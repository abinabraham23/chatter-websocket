...
function addModule ()
{
    if [ "$(ls -A ${JBOSS_BASE}/modules)" ]; then
        ...
        postgresql)
            ...
            $JBOSS_HOME/bin/jboss-cli.sh -c --command="module add --name=com.postgresql --resources=${JBOSS_BASE}/modules/${JDBC_JAR_NAME}.jar --dependencies=javax.api,javax.transaction.api"
            ;;
        mssql)
